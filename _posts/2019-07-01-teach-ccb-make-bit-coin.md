...
如果央行能够按照我上面写的去实现 DTC，我觉得 DTC 会是一种非常不错的数字货币, DTC 的生产方式是完全去中心化的，全世界的人和机构包括央行自己都可以通过挖矿的形式参与 DTC 的生产, DTC 的整个生产过程都是透明的，可以验证的, 并且包含了 POW. 由央行来作为 DTC 交易的记账中心将使交易的处理过程变得非常高效, 当然央行可能会做一些其它的事情比如 KYC(要求交易地址或者钱包实名制), 比如黑名单一些参与违法犯罪交易的地址， 在适当的程度下，我觉得这些措施都是可以接受的.
