...
<br/>
为了确认自己理解了比特币交易的数据结构，最好的方式是自己写一个 parser, 然后使用自己写的这个 parser 去解析比特币交易数据的内容。我们可以使用任何自己熟悉的编程语言去写这个 parser, 但是在这篇文章中我会用通篇使用 c 语言, 包括用 c 去写这个 parser, 因为我认为 c 语言是研究区块链技术最直接最方便的工具。
...
在这一节中我们将主要讨论解析 `tx_703fc5ee.bin` 这个文件的 `Txin-script / scriptSig`, `tx0.bin` 的 `Txin-script / scriptSig` 更准确的名字应该是 `coinbase`,

我们的代码 `parse_txin.c` 其实已经将 `coinbase` 完整地解析出来了，所以这一小节中不会涉及到文件 `tx0.bin` 的解析，至于怎么生成 `coinbase` 我们将在后面论述。
...
<div class="codes-comb" style="display:none;">
<button class="toggle-codes">隐藏代码</button>
<button class="copy-codes">Copy 代码</button>
</div>
