...
如果我们的机器本身是 Big-Endian 形式的那么上面的代码就会解析出一个错误的 tx_version, 如果是 Little-Endian 形式，那么程序可以解析出正确的 tx_version, 由于我们大部分的家用电脑使用的是 Intel CPU, 而 Intel CPU 都是 Little-Endian <sup>[[18]](#ref-18)</sup>, 所以上面的解析程序在我们自用的电脑上还是可以正常运行的, 如果考虑到跨平台运行，就一定要考虑到目标主机的 Endianness.
