...
我们使用两笔真实的比特币交易 [Tx-fe4](https://blockexplorer.com/api/tx/21f7c74240a92be7f95f8e8262849bc27cb3395859b0d6f82f23fc02619aefe4) 和 [Tx-bb4](https://blockexplorer.com/api/tx/0d7310ebcba3967f484a972fafd9746c40e626052cde959e860a8bb29e44bbb4) 作为验证对象.
...
spentTxId: "0d7310ebcba3967f484a972fafd9746c40e626052cde959e860a8bb29e44bbb4",
spentIndex: 10,
spentHeight: 446192
}
]
...
Tx-bb4 有众多的 vin, 其中有一个 vin 长下面这个样子, 这个 vin 包含的 txid 和 Tx-fe4 的 txid 是一样的, 说明这个 vin 的上家是 Tx-fe4, 我们将这个 vin 记为 vin-bb4-fe4,

```js
{
txid: "21f7c74240a92be7f95f8e8262849bc27cb3395859b0d6f82f23fc02619aefe4",
vout: 0,
scriptSig: {
asm: "30450221008738f6f97f3cafe1dfc79c63b7d33b356be47e5983f854203c59c45566a8c96802201190857c5dd7d1f63bb7ea30addb116ee7cdae8c588bf73df2dcf046e11888f4[ALL] 04e7e8eeb1d060e2ae611e2e6253b7948d7c02b7ea7c2dce09d939225a5c39d86553b07acf5073b1dc1e432cdf574d587b2800e0c3d4e43fcb3a1f84bfdf602a1e",
hex: ""
},
sequence: 4294967295,
n: 10,
...
1\. 解码 `1KFHE7w8BhaENAswwryaoccDb6qcT6DbYY` 然后做一个 scriptPubKey, 验证这个 scriptPubKey 能否和 `OP_DUP OP_HASH160 c825a1ecf2a6830c4401620c3a16f1995057c2ab OP_EQUALVERIFY OP_CHECKSIG` 一样.
...
输出: "scriptPubKey Hex : 76a914c825a1ecf2a6830c4401620c3a16f1995057c2ab88ac" 和 Tx-fe4 的 scriptPubKey Hex 是一样的.

2\. 将上面我们做出的 scriptPubKey 和 vin-bb4-fe4 中的 scriptSig 合并起来运行, 验证是否能够得到 true 值.
...
1\. 将来自于 scriptSig 的 \<pubKey\> 和来自于 scriptPubKey 的 \<pubKeyHash\> 进行比较看是否匹配, 而 \<pubKeyHash\> 由 address 生成，那么此逻辑可以概括为 \<pubKey\> 是否和 address 匹配;
...
上面的文字的简单解释就是: 首先 `text = sha256(sha256(Tx + 4 bytes hash type))`, 然后 `signature_verify(sig, text, pubKey)`, text 是被签名过的对象，如果是使用 pubKey 对应的私钥签名的话, 那么 `signature_verify(sig, text, pubKey)` 将返回 true 值.

第 1 条逻辑保证了 \<pubKey\> 是来自比特币的持有人, 第 2 条逻辑保证了比特币持有人用自己的私钥对 Tx 进行了签名, 即 \<sig\> 也来自比特币持有人.

现在问题来了上面英文中提到的 txCopy 具体指向哪笔交易? 这个 txCopy 是指向当前交易还是指向 vin 里包含的上笔交易? 为了回答这个问题，我们先看一张图:

[https://blockexplorer.com/api/tx/0d7310ebcba3967f484a972fafd9746c40e626052cde959e860a8bb29e44bbb4](https://blockexplorer.com/api/tx/0d7310ebcba3967f484a972fafd9746c40e626052cde959e860a8bb29e44bbb4)

![txsigmiss.jpg](/images/txsigmiss.jpg)

上图中被标注颜色的字符串是 vin 中各个 scriptSig 的 pubKey, 这些 pubKey 都是一样的, 但是 sig 却是不一样的, 如果 txCopy 指向的是当前交易, 相同的
pubKey 对应着同一个私钥, 同一个私钥对同一笔交易进行签名那么 sig 应该都是一样的, 但是实际情况是 vin 中各个 scriptSig 的 sig 签名都是不一样的, 所以我们可以推断 scriptSig 中的 sig 是通过对 vin 中 txid 所指向的交易签名得到的。

##### 2.2.2.4 生成 scriptSig

##### 2.2.2.5 执行脚本

##### 2.2.2.6 现在我们生成由自己掌控私钥的创世交易
