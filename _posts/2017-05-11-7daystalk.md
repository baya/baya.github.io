...
假设某个区块中有 T1, T2, T3, T4, T5 笔交易, 简单的做法是把 T1 至 T5 笔交易合并起来，然后做一个数字指纹, 比如 `SHA256(SHA256(T1 + T2 + T3 + T4 + T5))`, 其中 `+` 表示拼接的意思. 这样的做法有很多缺点, 我们会在后面讲述这些缺点，如果用 Merkle 树来实现这一功能，其计算过程如下所示:
