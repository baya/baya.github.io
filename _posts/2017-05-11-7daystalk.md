...
在这一节中我们将主要讨论解析 tx_703fc5ee.bin 和 tx0.bin 这两个文件的 Txin-script / scriptSig, 其中 tx0.bin 的 Txin-script / scriptSig 的更准确的名字应该是 coinbase script, 即属于 coinbase 交易的 script, coinbase 交易简单的理解就是创币交易，这种交易没有输入来源，用于生产比特币，伴随着区块的产生而产生，并且每个区块的第一笔交易必须是 coinbase 交易，并且只有这一笔 coinbase 交易.
